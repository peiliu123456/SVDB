...
from sa_loss import SASoftmaxLoss
import math
CE = nn.CrossEntropyLoss()


def sd(list_attentions_a, list_attentions_b, normalize=True, feature_distil_factor=None):
...
class SDA(nn.Module):
    def __init__(self, model, optimizer, args, steps=1, episodic=False):
        ...
        self.sa_loss = SASoftmaxLoss(self.model.head, num_classes=args.num_classes, scale=30.0, margin=0.4)
        self.optimizer_sa = torch.optim.SGD(self.sa_loss.parameters(), lr=1e-6)
    ...
    def forward_and_adapt(self, x, model, optimizer):
        ...
        labels = labels[mask]
        saloss = self.sa_loss(semantic[mask], labels)
        ...
        if self.freq != 0:
            ...
            sdloss = sd(hidden_layer, old_hidden_layer, feature_distil_factor=self.importance) * 10.
            loss += sdloss
        self.freq += 1
